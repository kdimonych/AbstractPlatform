...
struct CI2CBus
{
   ...
   * acknowledged, no device present.
   */
  inline int Write(std::uint8_t        aDeviceAddress,
                   const std::uint8_t* aDataSource,
                   size_t              aDataLength,
   ...
   * or no device present.
   */
  inline int Read(std::uint8_t  aDeviceAddress,
                  std::uint8_t* aDataDestination,
                  size_t        aDataLength,
                  bool          aNoStop) NOEXCEPT
  {
  ...
  inline bool ReadLastRegisterRaw(std::uint8_t aDeviceAddress,
                                  taTRegister& aRegisterValue,
                                  bool         aNoStop = false) NOEXCEPT
  {
    return iI2CBus.Read(aDeviceAddress,
                        reinterpret_cast<std::uint8_t*>(&aRegisterValue),
  ...
  bool ReadRegisterRaw(std::uint8_t       aDeviceAddress,
                       taTRegisterAddress aRegisterAddress,
                       taTRegister&       aRegisterValue,
                       bool               aNoStop = false) NOEXCEPT
  {
    return (iI2CBus.Write(aDeviceAddress, &aRegisterAddress, sizeof(aRegisterAddress), true)
   ...
   */
  template <typename taTRegisterAddress, typename taTRegister>
  bool WriteRegisterRaw(std::uint8_t       aDeviceAddress,
                        taTRegisterAddress aRegisterAddress,
                        taTRegister        aRegisterValue,
                        bool               aNoStop = false) NOEXCEPT
  {
    ...
    return iI2CBus.Write(aDeviceAddress, dataPack, sizeof(dataPack), aNoStop) != sizeof(dataPack);
  }
