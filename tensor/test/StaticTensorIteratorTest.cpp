...
struct StaticDimensionTest : public testing::Test
{
  ...
  using TStaticDimension = TStaticDimension<TSomeTag, kExpectedSize, kExpectedIterationDirection>;

  static constexpr size_t expectedPlainPosition(size_t forwardPosition)
  {
...
TYPED_TEST(StaticDimensionTest, InitialState)
{
  ...
  static_assert(
    std::is_same<decltype(TStaticDimension::Size()), typename TStaticDimension::TSize>::value);
  ...
  static_assert(TStaticDimension::kIterationDirection == TestFixture::kExpectedIterationDirection);
...
TYPED_TEST(StaticDimensionTest, RefInitialState)
{
  ...
  static_assert(
    std::is_same<typename TTStaticDimensionRef::TTag, typename TStaticDimension::TTag>::value);
  static_assert(
    std::is_same<typename TTStaticDimensionRef::TSize, typename TStaticDimension::TSize>::value);
  ...
  static_assert(std::is_same<decltype(dimensionRef.Size()), decltype(dimension.Size())>::value);
  static_assert(
    std::is_same<decltype(dimensionRef.GetPosition()), decltype(dimension.GetPosition())>::value);
  // TODO: check SetPosition() signature
  static_assert(
    std::is_same<decltype(dimensionRef.IsBegin()), decltype(dimension.IsBegin())>::value);
  static_assert(std::is_same<decltype(dimensionRef.IsLast()), decltype(dimension.IsLast())>::value);
  static_assert(std::is_same<decltype(dimensionRef.IsEnd()), decltype(dimension.IsEnd())>::value);
...
struct StaticTwoDimensionTensorIteratorTest : public testing::Test
{
  ...
  static constexpr auto ExpectedPlainPosition(size_t forwardPosition)
  {
...
TYPED_TEST_SUITE(StaticTwoDimensionTensorIteratorTest, TStaticTwoDimensionTensorIteratorTestTypes);
...
TYPED_TEST(StaticTwoDimensionTensorIteratorTest, SetGlobalIndex)
{
  ...
  auto                     setGlobalPositionTest = [&](size_t globalPositionIndex) {
    ...
    EXPECT_EQ(
      tesorIterator.template Dimension<0>().iDirectionalPosition,
      TestFixture::template ExpectedPlainPosition<TByte>((globalPositionIndex / 1) % TByte::kSize))
...
TEST(StaticTensorIteratorTest, SetGlobalIndexForThreeDimention)
{
  ...
  auto setGlobalPositionTest = [&](size_t globalPositionIndex) {
    ...
    EXPECT_EQ(
      ...
      expectedPosition((globalPositionIndex / kByteDimensionSize) % kWordDimensionSize, TWord{}))
      << " for the globalPositionIndex = " << globalPositionIndex;
    EXPECT_EQ(tesorIterator.Dimension<2>().iDirectionalPosition,
...
TEST(StaticTensorIteratorTest, TStaticTensorIteratorAsStaticDimension)
{
  ...
  auto               setGlobalPositionTest = [&](size_t globalPositionIndex) {
    ...
    EXPECT_EQ(tesorIterator.template Dimension<0>().GetPosition(), tesorIterator.GetPosition());
    EXPECT_EQ(tesorIterator.template Dimension<0>().template Dimension<0>().GetPosition(),
