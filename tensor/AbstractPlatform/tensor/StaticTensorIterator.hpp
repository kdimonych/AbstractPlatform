...
struct TStaticDimension
{
   ...
   */
  constexpr TPosition GetPosition() const
  {
    return kIterationDirection == TIterationDirection::Forward ? iDirectionalPosition
   ...
   */
  constexpr void SetPosition(TPosition aForwardPosition)
  {
  ...
  constexpr bool IsBegin()
  {
    return kIterationDirection == TIterationDirection::Forward ? iDirectionalPosition == 0
                                                               : iDirectionalPosition + 1 == kSize;
  }

  constexpr bool IsLast()
  {
    return kIterationDirection == TIterationDirection::Forward ? iDirectionalPosition + 1 == kSize
...
struct TStaticDimensionRef
{
   ...
   * @return constexpr TPosition
   */
  inline constexpr TPosition GetPosition() const
  {
   ...
   * @param aForwardPosition  The forward-directional position.
   */
  inline constexpr void SetPosition(TPosition aForwardPosition)
  {
    iDimension.SetPosition(aForwardPosition);
  }

  inline constexpr bool IsBegin()
  {
    return iDimension.IsBegin();
  }

  inline constexpr bool IsLast()
  {
    return iDimension.IsLast();
  }

  inline constexpr bool IsEnd()
  {
...
template <typename taStaticDimension>
static constexpr auto MakeStaticDimensionRef(taStaticDimension& aDimension)
{
...
struct TStaticDimensionTransformer
{
  ...
  static constexpr size_t DimentsionCount()
  {
  ...
  static constexpr TSize SubTensorSize()
  {
  ...
  inline static constexpr void Divide(TPosition aGlobalPosition, TDimensionList& aByDimensionList)
  {
    assert(aGlobalPosition <= Size());
    DevideImpl(aGlobalPosition, aByDimensionList, std::make_index_sequence<kDimentsionCount>{});
  }

  inline static constexpr TPosition Product(const TDimensionList& aDimensionList)
  {
...
private:
  ...
  static constexpr size_t SubTensorSizeImpl(std::integer_sequence<taT, taIndexes...>)
  {
  ...
  template <typename taT, taT... taIndexes>
  static constexpr void DevideImpl(TPosition       aGlobalPosition,
                                   TDimensionList& aByDimensionList,
                                   std::integer_sequence<taT, taIndexes...>)
  {
    ((std::get<taIndexes>(aByDimensionList)
        .SetPosition(aGlobalPosition / Divider<taIndexes>::Value() % Module<taIndexes>::Value())),
     ...);
  }
  ...
  static constexpr TPosition ProductImpl(const TDimensionList& aDimensionList,
                                         std::integer_sequence<taT, taIndex...>)
  {
    return ((std::get<taIndex>(aDimensionList).GetPosition() * Multiplier<taIndex>::Value()) + ...);
  }
};

template <typename... taDimensions>
static constexpr void DevideByDimensions(size_t aPosition, taDimensions&&... aByDimensions)
{
  using TStaticDimensionTransformer =
    TStaticDimensionTransformer<typename std::decay<taDimensions>::type&&...>;
  return TStaticDimensionTransformer::Divide(aPosition, std::forward_as_tuple(aByDimensions...));
}

template <typename... taDimensions>
inline static constexpr void DevideByDimensionsTuple(size_t                       aPosition,
                                                     std::tuple<taDimensions...>& aDimensions)
{
...
template <typename... taDimensions>
static constexpr auto DimensiosProduct(const taDimensions&... aDimensions)
{
  using TStaticDimensionTransformer =
    TStaticDimensionTransformer<typename std::decay<taDimensions>::type&&...>;
...
template <typename... taDimensions>
inline static constexpr auto DimensiosProductTuple(const std::tuple<taDimensions...>& aDimensions)
{
  using TStaticDimensionTransformer = TStaticDimensionTransformer<taDimensions...>;
...
public:
  ...
  template <size_t taDimentsionIndex>
  constexpr const auto& Dimension() const
  {
    static_assert(taDimentsionIndex < kDimentsionCount,
                  "The taDimentsionIndex must be less than kDimentsionCount");
  ...
  void SetPosition(size_t aGlobalPosition)
  {
